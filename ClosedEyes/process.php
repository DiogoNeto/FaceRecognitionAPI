		...
		if($opendir = opendir($dir)){
			...
			{ 
				if($dir+$file=="$fileList[$j]"){
					...
					$mysqli->query("INSERT INTO imagem (user,score,foto,data,criterio,session) VALUES ('user1',1,'$fileList[$j]',now(),'crit1',1)") or die($mysqli->error());
				}
			}
		}
	if($fileList[$j]==""){
		...
		if($opendir = opendir($dir)){
			...
			{ 
				if($dir+$file=="$fileList[$j]"){
					...
					//insere novo registo na tabela imagem
					$mysqli->query("INSERT INTO imagem (user,score,foto,data,criterio,session) VALUES ('user1',0,'$fileList[$j]',now(),'crit1',1)") or die($mysqli->error());
				}
			}
		}
	if($fileList[$j]==""){
