		...
		if($opendir = opendir($dir)){
			...
			{ 
				if($dir+$file=="$fileList[$j]"){
					...
					$mysqli->query("INSERT INTO imagem (user,score,foto,data,criterio,session) VALUES ('$user',1,'$fileList[$j]',now(),'$sessao',1)") or die($mysqli->error());
		...
		if($opendir = opendir($dir)){
			...
			{ 
				if($dir+$file=="$fileList[$j]"){
					...
					//insere novo registo na tabela imagem
					$mysqli->query("INSERT INTO imagem (user,score,foto,data,criterio,session) VALUES ('$user',0,'$fileList[$j]',now(),'$sessao',1)") or die($mysqli->error());
				}
			}
		}
	if($fileList[$j]==""){
